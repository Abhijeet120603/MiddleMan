...
#define AS7341_SPECTRAL_INT_HIGH_MSK 0x20
#define AS7341_SPECTRAL_INT_LOW_MSK  0x10
...
public:
    ...
    bool begin(uint8_t addr, I2C_HandleTypeDef *hi2c, int32_t id = 0);
    ...
    bool getAllChannels(uint32_t *buf);

    uint16_t readChannel(as7341_adc_channel_t ch);
    uint16_t getChannel(as7341_color_channel_t ch);

    void delayForData(uint32_t ms = 0);

    void powerEnable(bool en);
    void disableAll(void);
    bool enableSpectralMeasurement(bool en);

    bool setHighThreshold(uint16_t th);
    bool setLowThreshold(uint16_t th);
    uint16_t getHighThreshold(void);
    uint16_t getLowThreshold(void);

    bool enableSpectralInterrupt(bool en);
    bool enableSystemInterrupt(bool en);

    bool setAPERS(as7341_int_cycle_count_t count);
    bool setSpectralThresholdChannel(as7341_adc_channel_t ch);
    ...
    bool enableLED(bool en);
    bool setLEDCurrent(uint16_t ma);

    bool getIsDataReady(void);
    bool setBank(bool low);

    as7341_gpio_dir_t getGPIODirection(void);
    bool setGPIODirection(as7341_gpio_dir_t dir);
    bool getGPIOInverted(void);
    bool setGPIOInverted(bool inv);
    bool getGPIOValue(void);
    bool setGPIOValue(bool v);

protected:
    bool _init(int32_t id);

    I2C_HandleTypeDef *i2c_han = nullptr;
    uint8_t i2c_addr = 0;
    uint16_t _channel_readings[12] = {0};
    as7341_waiting_t _readingState;

private:
    bool enableSMUX(void);
    bool enableFlickerDetection(bool en);
    void FDConfig(void);
    int8_t getFlickerDetectStatus(void);
    bool setSMUXCommand(as7341_smux_cmd_t cmd);

    void setup_F1F4_Clear_NIR(void);
    void setup_F5F8_Clear_NIR(void);


    bool writeRegister(uint8_t reg, uint8_t *val, uint16_t len);
    bool writeRegisterByte(uint8_t reg, uint8_t val);
    bool readRegister(uint16_t reg, uint8_t *buf, uint16_t len);
    uint8_t readRegisterByte(uint16_t reg);

    bool modifyRegisterBit(uint16_t reg, bool val, uint8_t pos);
    bool modifyRegisterMultipleBit(uint16_t reg, uint8_t val, uint8_t pos, uint8_t bits);
    uint8_t modifyBitInByte(uint8_t v, uint8_t val, uint8_t pos);
